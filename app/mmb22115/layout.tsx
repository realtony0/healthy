      ...
      <div className="container-wide flex flex-col lg:flex-row gap-6 md:gap-12 items-start">
        ...
        <aside className="w-full lg:w-80 bg-white p-4 md:p-8 rounded-[2rem] md:rounded-[3.5rem] border border-gray-100 shadow-xl md:shadow-2xl shadow-gray-200/50 lg:sticky lg:top-32 top-24 space-y-6 md:space-y-10 max-h-screen lg:max-h-[calc(100vh-8rem)] overflow-y-auto">
            ...
            <nav className="space-y-2">
              {menu.map(item => (
                <Link 
                  key={item.href}
                  href={item.href} 
                  className="flex items-center gap-4 px-6 py-4 rounded-2xl font-bold text-gray-600 hover:bg-emerald-50 hover:text-[#1a472a] transition-all duration-300 group"
                >
                  <span className="text-gray-400 group-hover:text-[#1a472a] transition-colors">{item.icon}</span>
                  {item.label}
                </Link>
              ))}
            </nav>
          ...
          <div className="pt-8 border-t border-gray-100 space-y-4">
            <Link href="/" className="flex items-center gap-4 px-6 py-4 rounded-2xl font-bold text-gray-400 hover:text-gray-900 transition-colors">
              <Home size={20} />
              Retour au site
            </Link>
          </div>
