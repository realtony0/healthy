    ...
    <div className="pt-16 md:pt-20 pb-32">
      <div className="container-wide px-4 md:px-0">
        <div className="max-w-2xl mb-12 md:mb-20 space-y-4 text-center md:text-left">
          ...
          <h1 className="text-5xl lg:text-6xl font-black tracking-tight text-brand">Notre Carte</h1>
          <p className="text-lg text-gray-500 font-medium italic max-w-sm mx-auto md:mx-0">
        ...
        {categories.map((category) => (
          <section key={category.id} className="mb-16 md:mb-24 last:mb-0">
            <div className="flex items-center gap-4 mb-8 md:mb-10">
              <h2 className="text-xl md:text-2xl font-black text-brand uppercase tracking-widest text-xs">{category.name}</h2>
              <div className="h-px bg-gray-100 flex-grow" />
            </div>

            {category.type === 'BOWL_BUILDER' ? (
              <div className="bg-emerald-50 rounded-[3rem] p-8 lg:p-16 flex flex-col md:flex-row items-center gap-8 lg:gap-12 border border-emerald-100 shadow-xl shadow-emerald-900/5 overflow-hidden">
                <div className="flex-grow space-y-6 text-center md:text-left order-2 md:order-1">
                  <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-brand shadow-md mx-auto md:mx-0">
                    <Sparkles size={24} />
                  </div>
                  <h3 className="text-2xl md:text-3xl font-black text-brand">{category.name}</h3>
                  <p className="text-gray-500 text-base md:text-lg max-w-lg leading-relaxed italic">
                    Créez votre propre repas en choisissant votre base, vos protéines, vos légumes et vos sauces préférées.
                  </p>
                  <Link href="/menu/cree-ton-bowl" className="btn btn-primary btn-lg w-full md:w-fit px-10 py-5 text-lg">
                    Lancer la création
                    <ArrowRight className="w-5 h-5 ml-2" />
                  </Link>
                </div>
                <div className="w-full md:w-1/3 aspect-square rounded-[2rem] overflow-hidden shadow-2xl border-4 border-white order-1 md:order-2">
                  <Image 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-10 md:gap-y-12">
                {category.products.map((product) => (
                  <Link key={product.id} href={`/menu/${product.slug}`} className="group block">
                    <div className="aspect-square md:aspect-[4/5] rounded-[2.5rem] md:rounded-[2rem] overflow-hidden bg-gray-100 mb-6 relative shadow-xl shadow-gray-200/50">
                      {product.image ? (
                        <Image
                          src={product.image}
                          alt={product.name}
                    ...
                    <div className="space-y-2 px-2">
                      ...
                      <div className="flex items-center gap-2 text-brand font-black uppercase text-[10px] tracking-widest pt-2">
                        Détails <ArrowRight size={14} strokeWidth={3} className="group-hover:translate-x-1 transition-transform" />
                      </div>
                    </div>
                  </Link>
                ))}
